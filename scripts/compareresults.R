...
stata<-read_dta("/Volumes/COI/CData/chartbookdata_cbsa200910.dta")

sas <- sas %>% arrange(metro08, StatID, number)
stata <- stata %>% arrange(metro08, StatID, number)
...
stata1 <- stata %>% select(metro08, metronm, all_children,statname,StatID,number,children_imm_parents)
imm_par <- left_join(sas1,stata1,by=c("metro08", "metronm", "StatID", "number"))
